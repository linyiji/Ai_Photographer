...
export function renderPixels(source:SourceImage,recipe:AdjustmentRecipe,masks?:OptionalMaskSet):{data:Uint8ClampedArray;renderMs:number}{
 ...
 for(let y=0;y<source.height;y++)for(let x=0;x<source.width;x++){const pixel=y*source.width+x,index=pixel*4,pw=masks?.person?.[pixel]||0,bw=masks?.background?.[pixel]||0;const values:Values=all.map((v,i)=>v+person[i]*pw+background[i]*bw) as Values;for(const entry of local.values()){const weight=regionWeight((x+.5)/source.width,(y+.5)/source.height,entry.region);for(let i=0;i<4;i++)values[i]+=entry.values[i]*weight}let r=source.data[index],g=source.data[index+1],b=source.data[index+2];const stops=clamp(values[0])*.32;r=brightness(r,stops);g=brightness(g,stops);b=brightness(b,stops);const warm=clamp(values[1]);r+=warm*14;g+=warm*2;b-=warm*16;const saturation=1+clamp(values[2])*.28,luma=r*.2126+g*.7152+b*.0722;r=luma+(r-luma)*saturation;g=luma+(g-luma)*saturation;b=luma+(b-luma)*saturation;const softness=clamp(values[3]);if(softness){const factor=softness>=0?softness*.28:-softness*.12;if(softness>0){r+=(softened[index]-r)*factor;g+=(softened[index+1]-g)*factor;b+=(softened[index+2]-b)*factor}else{r+=(r-softened[index])*factor;g+=(g-softened[index+1])*factor;b+=(b-softened[index+2])*factor}}output[index]=byte(r);output[index+1]=byte(g);output[index+2]=byte(b);output[index+3]=source.data[index+3]}
