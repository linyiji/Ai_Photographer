...
export const regionWeight=(x:number,y:number,region:LocalRegion)=>{const bounded=region.width>=.04&&region.width<=1&&region.height>=.04&&region.height<=1&&region.x>=0&&region.x<=1-region.width&&region.y>=0&&region.y<=1-region.height&&region.feather>=.04&&region.feather<=.45,r=bounded?region:clampRegion(region);if(x<r.x||x>r.x+r.width||y<r.y||y>r.y+r.height)return 0;const lx=(x-r.x)/r.width,ly=(y-r.y)/r.height;return smooth(Math.min(lx,1-lx,ly,1-ly)/r.feather)}
...
const previewSoftenedCache=new WeakMap<Uint8ClampedArray,Uint8ClampedArray>()
export function renderPixels(source:SourceImage,recipe:AdjustmentRecipe,masks?:OptionalMaskSet,options?:{cacheSoftenedSource?:boolean}):{data:Uint8ClampedArray;renderMs:number}{
 ...
 for(const item of recipe.adjustments){const i=parameterIndex(item.parameter);if(item.scope==='ALL')all[i]+=item.value;else if(item.scope==='PERSON')person[i]+=item.value;else if(item.scope==='BACKGROUND')background[i]+=item.value;else if(item.region){const entry=local.get(item.region.id)||{region:clampRegion(item.region),values:empty()};entry.values[i]+=item.value;local.set(item.region.id,entry)}}
 const output=new Uint8ClampedArray(source.data.length),needsSoftened=recipe.adjustments.some(x=>x.parameter==='SOFTNESS'&&x.value!==0);let softened=source.data
 if(needsSoftened){if(options?.cacheSoftenedSource){softened=previewSoftenedCache.get(source.data)||boxBlur(source.data,source.width,source.height);previewSoftenedCache.set(source.data,softened)}else softened=boxBlur(source.data,source.width,source.height)}
