...
export default function ReplayLab(){
 const [scenarios,setScenarios]=useState<LabScenario[]>([]);const [scenario,setScenario]=useState('S01_HAPPY_PATH');const [mode,setMode]=useState('FROM_SCRATCH');const [result,setResult]=useState<ReplayResult|null>(null);const [busy,setBusy]=useState(false);const [error,setError]=useState('')
 useLoad(()=>{if(__XFX_LAB_MODE__)api.labScenarios().then(setScenarios).catch(e=>setError(String(e)))})
 ...
 const run=async()=>{setBusy(true);setError('');try{setResult(await api.runReplay(scenario,mode,mode==='FROM_CHECKPOINT'?5:undefined))}catch(e){setError(e instanceof Error?e.message:String(e))}finally{setBusy(false)}}
