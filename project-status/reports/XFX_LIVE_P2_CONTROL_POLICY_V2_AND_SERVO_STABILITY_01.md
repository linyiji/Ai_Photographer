...
Gate 1 Attempt 5 supplies four post-copy-fix traces. X issuance is restored (`MOVE_RIGHT ×2` in Trial 3), all four trials reach READY with 5.56–7.46 seconds of post-READY observation, post-READY ordinary actions are zero, and no trace alternates X or Scale direction. It is not an accepted Gate 1 sample: no trial contains both X and Scale precision corrections, and device performance regressed to 3.79–5.04 Hz local trace cadence with inference p95 261.4–377.5 ms, skipped-busy 35.6%–45.8%, and 28–126 stale-suppressed rows. Preview remains 27.2–30.0 fps and all external/raw counters are zero. Because the same device had just sustained 6.95–7.21 Hz and warmed p95 86.9–108.0 ms while the only intervening implementation change was presentation copy, no inference/control change is inferred. Revalidation from one fresh browser tab is required; Gate 2 remains blocked.

Gate 1 Attempt 6 supplies four correctly labeled traces. One X Episode and one Scale Episode terminate SUCCESS; all four trials reach READY, retain 5.66–6.87 seconds of observation, emit zero post-READY ordinary action, and contain no direction alternation. The labeled Combined candidate contains only a precision `MOVE_CLOSER` Episode and no X Episode, so the required combined X+Scale coverage remains absent. Performance recovers to 6.28–7.01 Hz scenario-local cadence, cumulative Vision 6.815–6.870 Hz, Preview 29.0–30.1 fps, and 4.5%–5.1% skipped-busy. Inference p95 remains variable at 108.4–142.0 ms, with two of four snapshots above the 120 ms candidate; the user reports the phone becomes hot after sustained use and p95 is often around 140 ms with later periods near 90 ms. This evidence is not Gate 1 PASS, and the longer Gate 2 run remains blocked pending a real Combined trial and subjective hard-invariant confirmation.

Gate 1 Attempt 7 adds labeled Scale and Combined supplements. Scale performs only the coarse `HEAD_SHOULDERS / TOO_TIGHT` compatibility transition and then reaches passive READY, with no precision Scale Episode. Combined produces four X Episodes and no Scale Episode, so it is still not combined-axis coverage. The X sequence is `LEFT -> RIGHT -> RIGHT -> LEFT`: Anchor X crosses from about 0.390 to 0.680 in Episode 1, later crosses from about 0.674 through the STOP point near 0.509 to about 0.380 in Episode 3, and the first three Episodes terminate `NO_EFFECT`; READY is later reached by passive confirmation near Anchor X 0.502. Post-READY ordinary output remains zero. Performance is substantially healthy (trace cadence 6.69/7.02 Hz, Preview 28.0/29.8 fps, Vision 6.155/6.627 Hz, skipped-busy 3.4%/2.8%); inference p95 is 124.8 ms for Scale and 111.3 ms for Combined. This is not Gate 1 PASS: precision Scale and true Combined coverage are absent, obvious X alternation is present, and the prior sustained-use thermal warning remains open. Gate 2 remains blocked; no Target, Deadband, threshold, or success semantic is changed.

Attempt 8 begins with acceptance-harness hardening only. The debug page now blocks invalid X/Scale/Combined ARM conditions by reusing the unchanged Target, Deadband, BodyMode, Precision Scale calibration, and measurement validity. The blocked state is visible inline beside the button (`ARM 条件未满足` / `ARM 已阻止` plus exact failure reasons), rather than only in the remote Camera message region. A valid ARM locks Pre-ARM state and the trace reconstructs actual coverage, success counts, READY causality, post-READY output, and STOP-to-settle scalar response without raw media. OPPO screenshots then exposed a presentation ambiguity: one state had a precision-valid X (`0.554 / TOO_RIGHT`) and invalid Scale, while a later state had a displayable X anchor (`0.599`) but precision-invalid X and Scale; both were still incompatible `HEAD_SHOULDERS`. The harness now exposes the unchanged X band and BodyMode-specific semantic Scale band, suppresses redundant range failures when a measurement is invalid, gives an actionable `UPPER_BODY` instruction, and no longer compares semantic Scale against the legacy `height_ratio=0.350` display. This changes no Target, Deadband, calibration, or acceptance semantic. Automated tests are 216/216 PASS; TypeScript and production build / 30 modules pass. Browser Replay reaches READY from EPISODE_SUCCESS with ordinary/STOP/HOLD/success `1/1/1/1`, zero console errors, and all external counters zero; invalid-ARM smoke remains DISARMED. Fresh OPPO X, Scale, and Combined traces are still required before any Gate 1 or overshoot decision. No controller change or post-STOP barrier is authorized by current evidence.
