...
import { runV4BrowserScenario, runV4TargetScopedCropBrowserGate, runV4XDirectionBrowserGate, V4_BROWSER_MATRIX } from '../control-v4/browser-scenarios.js';
...
function renderV4Control():void{const snapshot=latestV4Snapshot;const observation=latestV4Observation??snapshot?.observation??null;const presentation=deriveV4Presentation(snapshot),calibration=v4CalibrationLabel(),calibrationCopy=calibration?(latestV4Calibration?.copy_zh??`标定未开始：点击按钮后，先静止，再向你自己的${calibration==='LEFT'?'左':'右'}侧移动一步并停稳`):null,singleComplete=v4SingleXDeviceGate()&&Boolean(v4SingleGateResult)&&!v4ArmRequested,singleCopy=singleComplete?`本轮 05F 单次 X Episode 已完成：${v4SingleGateResult}。请下载标量 Trace。`:null,armPendingCopy=v4ArmRequested&&!snapshot?'05F 单次 X 复验已 ARM，正在等待第一帧有效观察。':null;v3DebugPanel.hidden=false;v2DebugGrid.hidden=true;v3Fields.policy.textContent=`CONTROL POLICY · V4 · ${snapshot?.target.id??currentV4Target.id}`;v3Fields.stage.textContent=calibration?`X_CALIBRATION · ${latestV4Calibration?.phase??'IDLE'}`:singleComplete?'SINGLE_X_COMPLETE':armPendingCopy?'ARMED_WAITING_OBSERVATION':snapshot?.stage??'ACQUIRE_SUBJECT';v3Fields.framing.textContent=snapshot?.constraints.scale_relation??'UNKNOWN';v3Fields.x.textContent=snapshot?.constraints.x_relation??'UNKNOWN';v3Fields.measurement.textContent=observation?`RECOGNIZED ${observation.subject_recognition.detected?'YES':'NO'} · OBSERVED ${observation.observed_body.coverage} · GAP ${snapshot?.target_gap.ready?'READY':'OPEN'}`:'RECOGNIZED NO · OBSERVED UNKNOWN · GAP OPEN';v3Fields.stable.textContent=observation?.stable?'YES':'NO';v3Fields.action.textContent=calibration?'NONE · CALIBRATION ONLY':snapshot?.active_episode?.action??snapshot?.action??'—';v3Fields.outcome.textContent=singleComplete?v4SingleGateResult!:snapshot?.last_episode?.outcome??(snapshot?.active_episode?.no_response_recorded?'NO_RESPONSE':'—');v3Fields.latency.textContent=`${percentile(v4ControllerLatencies,.5).toFixed(3)} / ${percentile(v4ControllerLatencies,.95).toFixed(3)} ms`;closedLoopFields.runtimeState.textContent=`STATE · ${calibration?latestV4Calibration?.phase??'X_CALIBRATION':singleComplete?'SINGLE_X_COMPLETE':armPendingCopy?'ARMED_WAITING_OBSERVATION':snapshot?.stage??'IDLE'}`;closedLoopFields.ready.textContent=`ARMED · ${String(snapshot?.armed??v4ArmRequested).toUpperCase()} · READY · ${String(snapshot?.ready??false).toUpperCase()}`;closedLoopFields.instruction.textContent=calibrationCopy??singleCopy??armPendingCopy??presentation.primary_copy_zh;guidanceOverlayState.textContent=calibration?`V4 · X CALIBRATION · ${latestV4Calibration?.phase??'IDLE'}`:singleComplete?'V4 · SINGLE X COMPLETE':armPendingCopy?'V4 · ARMED':`V4 · ${presentation.state}`;guidanceOverlayText.textContent=calibrationCopy??singleCopy??armPendingCopy??presentation.primary_copy_zh;closedLoopReset.textContent='重置 V4 试验';closedLoopArm.textContent=calibration?(latestV4Calibration?.phase==='COMPLETE'?'重新开始 X 标定':latestV4Calibration?.active?'重新开始 X 标定':'开始 X 标定记录'):v4SingleXDeviceGate()?(singleComplete?'重新开始 05F 单次 X 复验':'ARM 05F 单次 X 复验'):'ARM V4 新试验';}
