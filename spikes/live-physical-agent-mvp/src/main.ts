...
const perceptionRuntime = new PerceptionRuntime({
  ...
  onState: (state, rawMeasurement) => {
    ...
    if(activePolicy()==='V4'){
      const decisionAt=performance.now();latestV4Observation=v4Projector.project(state,Math.max(0,decisionAt-state.timestamp_ms));const started=performance.now();latestV4Snapshot=v4Controller.update(latestV4Observation,v4DirectionContext());v4ArmRequested=latestV4Snapshot.armed;v4ControllerLatencies.push(performance.now()-started);if(v4ControllerLatencies.length>240)v4ControllerLatencies.shift();if(v4CalibrationLabel())latestV4Calibration=v4XCalibration.update(latestV4Observation);v4Trace.append(latestV4Snapshot,latestV4Calibration);if(latestV4Calibration?.phase==='COMPLETE'&&!v4CalibrationCompletionReported){v4CalibrationCompletionReported=true;setMessage(`${latestV4Calibration.copy_zh}。请下载标量 Trace。`);}if(v4SingleXDeviceGate()&&latestV4Snapshot.last_episode?.stage==='ALIGN_PRIMARY_ANCHOR'&&latestV4Snapshot.last_episode.state==='EVALUATED'&&!v4SingleGateResult){v4SingleGateResult=latestV4Snapshot.last_episode.outcome??'UNKNOWN';v4Controller.reset();v4ArmRequested=false;setMessage(`05F 单次 X Episode 已结束：${v4SingleGateResult}。本轮已自动解除，请下载标量 Trace。`);}latestV4SubjectBox=v4VisualProjector.projectSubjectOnly(state,decisionAt);latestClosedLoop=null;latestVisualGuidance=null;
...
closedLoopArm.addEventListener('click', () => {
  if(activePolicy()==='V4'){const calibration=v4CalibrationLabel(),now=performance.now();if(calibration){v4Controller.reset();v4ArmRequested=false;v4Trace.clear();v4CalibrationCompletionReported=false;v4SingleGateResult=null;latestV4Calibration=v4XCalibration.start(calibration,latestV4Observation?.timestamp_ms??now);if(latestV4Observation){latestV4Snapshot=v4Controller.update(latestV4Observation,v4DirectionContext());latestV4Calibration=v4XCalibration.update(latestV4Observation);v4Trace.append(latestV4Snapshot,latestV4Calibration);}renderClosedLoop();renderVisualGuidance();setMessage(`X 标定已开始：只需让头部和双肩清晰可见并保持不动；界面明确提示“基线已取得”后，再向你自己的${calibration==='LEFT'?'左边':'右边'}移动一小步并停稳。`);return;}v4Controller.arm(now);v4ArmRequested=true;v4Trace.clear();v4XCalibration.reset();latestV4Calibration=null;v4CalibrationCompletionReported=false;v4SingleGateResult=null;if(latestV4Observation){latestV4Snapshot=v4Controller.update(latestV4Observation,v4DirectionContext());v4ArmRequested=latestV4Snapshot.armed;v4Trace.append(latestV4Snapshot);}else latestV4Snapshot=null;renderClosedLoop();renderVisualGuidance();setMessage(v4SingleXDeviceGate()?'05F 单次 X 复验已 ARM：Scale 保持目标范围，按唯一一条左右指令移动并停稳；Episode 评估后会自动解除控制。':'V4 试验已 ARM：一次只响应一条小步指令；无响应只提醒，不会被判成结果。');return;}
