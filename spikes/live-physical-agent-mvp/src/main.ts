...
import type { VoiceTelemetryV01 } from '../control-v4/voice-types.js';
import { ALL_LIVE_TARGET_FIXTURES_V04, DEFAULT_LIVE_TARGET_V02, LIVE_TARGET_FIXTURES_V02 } from '../control-v4/targets.js';
import { V4ScalarTraceRecorder } from '../control-v4/trace.js';
import type { HumanObservationV02, LiveTargetV02, TargetFixtureIdV02, V4DirectionContextV01, V4Snapshot } from '../control-v4/types.js';
import { SubjectXCalibrationSessionV01, type SubjectXCalibrationSnapshotV01 } from '../control-v4/x-calibration.js';
import { runV4BrowserScenario, runV4FramingPositionScenario, runV4TargetScopedCropBrowserGate, runV4XDirectionBrowserGate, V4_BROWSER_MATRIX, V4_FRAMING_POSITION_MATRIX } from '../control-v4/browser-scenarios.js';
import { runCanonicalGuidanceBrowserMatrixV01 } from '../control-v4/canonical-browser-scenarios.js';
import { canonicalTargetFromFixtureV01, evaluateControlFeasibilityV01 } from '../control-v4/canonical-guidance.js';
...
const voiceGuidance = requireElement<HTMLInputElement>('voice-guidance');
...
const canonicalGuidanceHud=requireElement<HTMLElement>('canonical-guidance-hud');
const canonicalFields={target:requireElement<HTMLElement>('cg-target'),feasibility:requireElement<HTMLElement>('cg-feasibility'),error:requireElement<HTMLElement>('cg-error'),owner:requireElement<HTMLElement>('cg-owner'),command:requireElement<HTMLElement>('cg-command'),ready:requireElement<HTMLElement>('cg-ready'),authority:requireElement<HTMLElement>('cg-authority'),voice:requireElement<HTMLElement>('cg-voice')};
...
let latestV4Calibration:SubjectXCalibrationSnapshotV01|null=null;
let v4CalibrationCompletionReported=false;
let v4SingleGateResult:string|null=null;
let v4ArmRequested=false;
let v4ControllerLatencies:number[]=[];
let latestV4SubjectBox:NormalizedBox|null=null;
let latestVoiceTelemetry:Readonly<VoiceTelemetryV01>|null=null;
let v4VoiceSessionRevision=1;
...
let currentV4Target:LiveTargetV02=DEFAULT_LIVE_TARGET_V02;
...
const v4VoiceAdapter=new BrowserSpeechSynthesisVoiceAdapterV01();
const v4VoiceEngine=new VoiceCueEngineV01(v4VoiceAdapter);
latestVoiceTelemetry=v4VoiceEngine.snapshot();
...
else if(controlPolicySelect.value==='V4')scaleGateScenario.value=fullDebugRequested?'V4_UPPER_BODY_CENTER':'V4_HEAD_SHOULDERS_LEFT_TOP';
...
const v4SingleXDeviceGate=():boolean=>scaleGateScenario.value==='V4_X_DEVICE_SINGLE_STEP';
const v4TargetForScenario=(value:string):LiveTargetV02=>ALL_LIVE_TARGET_FIXTURES_V04[value.replace(/^V4_/,'')]??DEFAULT_LIVE_TARGET_V02;
...
const perceptionRuntime = new PerceptionRuntime({
  ...
  onState: (state, rawMeasurement) => {
    ...
    if(activePolicy()==='V4'){
      const decisionAt=performance.now();latestV4Observation=v4Projector.project(state,Math.max(0,decisionAt-state.timestamp_ms));const started=performance.now();latestV4Snapshot=v4Controller.update(latestV4Observation,v4DirectionContext());v4ArmRequested=latestV4Snapshot.armed;v4ControllerLatencies.push(performance.now()-started);if(v4ControllerLatencies.length>240)v4ControllerLatencies.shift();if(v4CalibrationLabel())latestV4Calibration=v4XCalibration.update(latestV4Observation);const voicePresentation=deriveV4Presentation(latestV4Snapshot);latestVoiceTelemetry=v4VoiceEngine.update(voicePresentation,{now:decisionAt,session_revision:v4VoiceSessionRevision,armed:latestV4Snapshot.armed,target_id:latestV4Snapshot.target.id,current_framing_ready:latestV4Snapshot.current_framing_ready,trial_success_latched:latestV4Snapshot.trial_success_latched});v4Trace.append(latestV4Snapshot,latestV4Calibration,latestVoiceTelemetry);if(latestV4Calibration?.phase==='COMPLETE'&&!v4CalibrationCompletionReported){v4CalibrationCompletionReported=true;setMessage(`${latestV4Calibration.copy_zh}。请下载标量 Trace。`);}if(v4SingleXDeviceGate()&&latestV4Snapshot.last_episode?.stage==='ALIGN_PRIMARY_ANCHOR'&&latestV4Snapshot.last_episode.state==='EVALUATED'&&!v4SingleGateResult){v4SingleGateResult=latestV4Snapshot.last_episode.outcome??'UNKNOWN';v4Controller.reset();v4ArmRequested=false;v4VoiceEngine.invalidate('LIVE_DISARMED');latestVoiceTelemetry=v4VoiceEngine.snapshot();setMessage(`05F 单次 X Episode 已结束：${v4SingleGateResult}。本轮已自动解除，请下载标量 Trace。`);}latestV4SubjectBox=v4VisualProjector.projectSubjectOnly(state,decisionAt);latestClosedLoop=null;latestVisualGuidance=null;
...
function renderGate1Precondition(): void {
  ...
  gate1Precondition.hidden = activePolicy()!=='V2'||!isGate1Scenario(scenario);
...
function renderVisualGuidance(): void {
  if(activePolicy()==='V4'){renderV4Visual();return;}
...
function renderV4Visual():void{const snapshot=latestV4Snapshot,presentation=deriveV4Presentation(snapshot),calibration=v4CalibrationLabel();visualOverlay.dataset.status=presentation.state==='READY'?'READY':presentation.action?'CORRECTING':presentation.state==='ACQUIRING'?'LOST':'TRACKING';visualGrid.classList.toggle('is-visible',guidanceGrid.checked);stopVisual.classList.remove('is-visible');const showSubjectBox=Boolean(snapshot?.observation.subject_recognition.lock_state==='LOCKED');applyProjectedBox(subjectBox,showSubjectBox?latestV4SubjectBox:null);const target=snapshot?.canonical_guidance.canonical_target,legacyTarget=snapshot?.target??currentV4Target,targetX=target?.anchor.x??legacyTarget.target_anchor_x,targetY=target?.anchor.y??legacyTarget.target_anchor_y??.5,targetHeight=Math.min(.9,Math.max(.14,target?.scale.target??legacyTarget.target_scale)),targetWidth=Math.min(.82,Math.max(.16,targetHeight*.62)),targetVisual:NormalizedBox={left:targetX-targetWidth/2,top:targetY-targetHeight/2,width:targetWidth,height:targetHeight,center_x:targetX,center_y:targetY},zoneHeight=Math.max(.06,(target?.anchor.tolerance_y??legacyTarget.tolerance_y??.07)*2),zoneWidth=Math.max(.06,(target?.anchor.tolerance_x??legacyTarget.tolerance_x)*2),zoneVisual:NormalizedBox={left:targetX-zoneWidth/2,top:targetY-zoneHeight/2,width:zoneWidth,height:zoneHeight,center_x:targetX,center_y:targetY};applyProjectedBox(targetBox,targetVisual);applyProjectedBox(acceptableZone,zoneVisual);debugPoseBox.classList.remove('is-visible');semanticAnchor.classList.remove('is-visible');semanticDebugCard.hidden=true;subjectLockLabel.textContent=showSubjectBox?`观察框 · ${snapshot?.observation.observed_body.observed_extent??'UNKNOWN'}`:'人物锁定中';if(calibration){readyVisual.classList.remove('is-visible');directionVisual.classList.remove('is-visible');trackingVisual.textContent=latestV4Calibration?.copy_zh??`X 标定：点击开始后，先静止，再向你自己的${calibration==='LEFT'?'左边':'右边'}移动一小步并停稳`;return;}readyVisual.classList.toggle('is-visible',presentation.state==='READY');directionVisual.classList.toggle('is-visible',Boolean(presentation.action));if(presentation.action){directionLabel.textContent=presentation.overlay_copy_zh;const horizontal=presentation.action.includes('LEFT')||presentation.action.includes('RIGHT');directionIcon.textContent=horizontal?(presentation.display_axis_sign<0?'←':'→'):presentation.action==='MOVE_CLOSER_SMALL'?'＋':presentation.action==='MOVE_FARTHER_SMALL'?'−':presentation.action==='MOVE_CAMERA_UP_SMALL'?'↑':'↓';}trackingVisual.textContent=v4ArmRequested&&!snapshot?'V4 已 ARM，等待第一帧有效观察':presentation.overlay_copy_zh;}
...
function renderV3Control():void{
  canonicalGuidanceHud.hidden=true;
...
function renderV4Control():void{
  const snapshot=latestV4Snapshot,observation=latestV4Observation??snapshot?.observation??null,presentation=deriveV4Presentation(snapshot),calibration=v4CalibrationLabel();
  const calibrationCopy=calibration?(latestV4Calibration?.copy_zh??`标定未开始：点击按钮后，先静止，再向你自己的${calibration==='LEFT'?'左边':'右边'}移动一小步并停稳`):null;
  const singleComplete=v4SingleXDeviceGate()&&Boolean(v4SingleGateResult)&&!v4ArmRequested,singleCopy=singleComplete?`本轮 05F 单次 X Episode 已完成：${v4SingleGateResult}。请下载标量 Trace。`:null,armPendingCopy=v4ArmRequested&&!snapshot?(v4SingleXDeviceGate()?'05F 单次 X 复验已 ARM，正在等待第一帧有效观察。':'V4 已 ARM，正在等待第一帧有效观察。'):null;
  v3DebugPanel.hidden=false;v2DebugGrid.hidden=true;canonicalGuidanceHud.hidden=false;
  v3Fields.policy.textContent=`PROFILE · ${snapshot?.target.framing_profile_id??currentV4Target.framing_profile_id} · ZONE · ${snapshot?.target.position_zone??currentV4Target.position_zone}`;
  v3Fields.stage.textContent=calibration?`X_CALIBRATION · ${latestV4Calibration?.phase??'IDLE'}`:singleComplete?'SINGLE_X_COMPLETE':armPendingCopy?'ARMED_WAITING_OBSERVATION':snapshot?.stage??'ACQUIRE_SUBJECT';
  v3Fields.framing.textContent=`SCALE ${snapshot?.constraints.scale_relation??'UNKNOWN'} · Y ${snapshot?.constraints.y_zone_relation??'UNKNOWN'}`;
  v3Fields.x.textContent=snapshot?.constraints.x_zone_relation??'UNKNOWN';
  v3Fields.measurement.textContent=observation?`RECOGNIZED ${observation.subject_recognition.detected?'YES':'NO'} · EXTENT ${observation.observed_body.observed_extent} · GAP ${snapshot?.target_gap.ready?'READY':'OPEN'}`:'RECOGNIZED NO · EXTENT UNKNOWN · GAP OPEN';
  v3Fields.stable.textContent=observation?.stable?'YES':'NO';v3Fields.action.textContent=calibration?'NONE · CALIBRATION ONLY':snapshot?.active_episode?.action??snapshot?.action??'—';v3Fields.outcome.textContent=singleComplete?v4SingleGateResult!:snapshot?.last_episode?.outcome??(snapshot?.active_episode?.no_response_recorded?'NO_RESPONSE':'—');v3Fields.latency.textContent=`${percentile(v4ControllerLatencies,.5).toFixed(3)} / ${percentile(v4ControllerLatencies,.95).toFixed(3)} ms`;
  closedLoopFields.runtimeState.textContent=`STATE · ${calibration?latestV4Calibration?.phase??'X_CALIBRATION':singleComplete?'SINGLE_X_COMPLETE':armPendingCopy?'ARMED_WAITING_OBSERVATION':snapshot?.stage??'IDLE'}`;
  closedLoopFields.ready.textContent=`ARMED · ${String(snapshot?.armed??v4ArmRequested).toUpperCase()} · CURRENT READY · ${String(snapshot?.current_framing_ready??false).toUpperCase()} · TRIAL SUCCESS · ${String(snapshot?.trial_success_latched??false).toUpperCase()}`;
  closedLoopFields.instruction.textContent=calibrationCopy??singleCopy??armPendingCopy??presentation.primary_copy_zh;guidanceOverlayState.textContent=calibration?`V4 · X CALIBRATION · ${latestV4Calibration?.phase??'IDLE'}`:singleComplete?'V4 · SINGLE X COMPLETE':armPendingCopy?'V4 · ARMED':`V4 · ${presentation.state}`;guidanceOverlayText.textContent=calibrationCopy??singleCopy??armPendingCopy??presentation.primary_copy_zh;closedLoopReset.textContent='重置 V4 试验';closedLoopArm.textContent=calibration?(latestV4Calibration?.phase==='COMPLETE'?'重新开始 X 标定':latestV4Calibration?.active?'重新开始 X 标定':'开始 X 标定记录'):v4SingleXDeviceGate()?(singleComplete?'重新开始 05F 单次 X 复验':'开始 05F 单次 X 复验'):'开始 V4 引导';
  const guidance=snapshot?.canonical_guidance,command=guidance?.command,error=guidance?.error_vector.dimensions;if(guidance&&error){canonicalFields.target.textContent=`${guidance.canonical_target.target_id} / r${guidance.canonical_target.target_revision}`;canonicalFields.feasibility.textContent=`${guidance.canonical_target.status} / ${guidance.feasibility.status} · ${guidance.feasibility.execution_class}`;canonicalFields.error.textContent=`${error.EXTENT.relation} / ${error.SCALE.relation} / ${error.X.relation} / ${error.Y.relation}`;canonicalFields.owner.textContent=`${command?.owner??'—'} / ${command?.action??'—'}`;canonicalFields.command.textContent=`${command?.command_id??'—'} / ${command?.lifecycle??'—'}`;canonicalFields.ready.textContent=`${String(guidance.verification.target_reached).toUpperCase()} / ${String(guidance.verification.verified).toUpperCase()} / ${String(snapshot.current_framing_ready).toUpperCase()}`;canonicalFields.authority.textContent=`${guidance.primary_authority} / ${guidance.secondary_authority}`;canonicalFields.voice.textContent=guidance.voice_event??'NONE';}else{const candidateTarget=canonicalTargetFromFixtureV01(currentV4Target,1),feasibility=evaluateControlFeasibilityV01(candidateTarget);canonicalFields.target.textContent=`${candidateTarget.target_id} / r${candidateTarget.target_revision}`;canonicalFields.feasibility.textContent=`${candidateTarget.status} / ${feasibility.status} · ${feasibility.execution_class}`;canonicalFields.error.textContent='UNKNOWN / UNKNOWN / UNKNOWN / UNKNOWN';canonicalFields.owner.textContent='— / —';canonicalFields.command.textContent='— / —';canonicalFields.ready.textContent='FALSE / FALSE / FALSE';canonicalFields.authority.textContent='CANONICAL_COMMAND / OBSERVATION_AND_COMMAND_PROGRESS';canonicalFields.voice.textContent='NONE';}
  const voice=latestVoiceTelemetry??v4VoiceEngine.snapshot();voiceStatus.textContent=`VOICE · ${voice.voice_enabled?'ON':'OFF'} · ${voice.voice_available?voice.voice_prepared?voice.voice_state:'UNPREPARED':'UNAVAILABLE'}`;document.documentElement.dataset.voiceEnabled=String(voice.voice_enabled);document.documentElement.dataset.voiceAvailable=String(voice.voice_available);document.documentElement.dataset.voiceState=voice.voice_state;document.documentElement.dataset.voiceSemantic=voice.voice_semantic_id??'NONE';
}
...
function renderClosedLoop(): void {
  v3DebugPanel.hidden=activePolicy()==='V2';v2DebugGrid.hidden=activePolicy()!=='V2';canonicalGuidanceHud.hidden=activePolicy()!=='V4';
  if(activePolicy()==='V4'){renderV4Control();return;}
...
function stopCamera(options: { preserveMessage?: boolean } = {}): void {
  v4VoiceSessionRevision+=1;v4VoiceEngine.invalidate('LIVE_DISARMED');latestVoiceTelemetry=v4VoiceEngine.snapshot();
...
async function startCamera(facingMode: FacingMode): Promise<void> {
  ...
  v4VoiceSessionRevision+=1;v4VoiceEngine.invalidate('STALE_SESSION');latestVoiceTelemetry=v4VoiceEngine.snapshot();
...
voiceGuidance.addEventListener('change',()=>{v4VoiceEngine.setEnabled(voiceGuidance.checked);latestVoiceTelemetry=v4VoiceEngine.snapshot();renderClosedLoop();setMessage(voiceGuidance.checked?'语音引导已开启；下次开始 V4 引导后生效。':'语音引导已关闭；文字、视觉和控制保持不变。');});
scaleGateScenario.addEventListener('change', () => { activeGate1PreArm = null; gate1ArmAttempted = false;if(activePolicy()==='V4'&&scaleGateScenario.value.startsWith('V4_')){v4VoiceSessionRevision+=1;v4VoiceEngine.invalidate('STALE_TARGET');latestVoiceTelemetry=v4VoiceEngine.snapshot();currentV4Target=v4CalibrationLabel()||v4SingleXDeviceGate()?LIVE_TARGET_FIXTURES_V02.CENTER_UPPER_BODY:v4TargetForScenario(scaleGateScenario.value);v4Controller.setTarget(currentV4Target);v4ArmRequested=false;v4XCalibration.reset();latestV4Calibration=null;v4CalibrationCompletionReported=false;v4SingleGateResult=null;v4Trace.clear();latestV4Snapshot=null;closedLoopArm.textContent=v4CalibrationLabel()?'开始 X 标定记录':v4SingleXDeviceGate()?'开始 05F 单次 X 复验':'开始 V4 引导';}renderGate1Precondition();renderClosedLoop();renderVisualGuidance(); });
controlPolicySelect.addEventListener('change',()=>{v4VoiceSessionRevision+=1;v4VoiceEngine.invalidate('LIVE_DISARMED');latestVoiceTelemetry=v4VoiceEngine.snapshot();closedLoop.reset();v3Controller.reset();v3Projector.reset();v4Controller.reset();v4Projector.reset();v4VisualProjector.reset();v4XCalibration.reset();scalarTrace.clear();v3Trace.clear();v4Trace.clear();latestClosedLoop=null;latestV3Measurement=null;latestV3Snapshot=null;latestV4Observation=null;latestV4Snapshot=null;latestV4Calibration=null;v4CalibrationCompletionReported=false;v4SingleGateResult=null;v4ArmRequested=false;latestV4SubjectBox=null;latestVisualGuidance=null;v3ControllerLatencies=[];v4ControllerLatencies=[];displayedActionCopy=null;displayedActionUntilMs=0;activeGate1PreArm=null;gate1ArmAttempted=false;semanticDebug.disabled=activePolicy()==='V4';if(activePolicy()==='V4'){semanticDebug.checked=false;if(!scaleGateScenario.value.startsWith('V4_'))scaleGateScenario.value='V4_CENTER_UPPER_BODY';currentV4Target=v4TargetForScenario(scaleGateScenario.value);v4Controller.setTarget(currentV4Target);}else if(activePolicy()==='V3'&&!scaleGateScenario.value.startsWith('V3_'))scaleGateScenario.value='V3_FRAMING_ONLY';visualProjector.reset();renderClosedLoop();renderGate1Precondition();renderVisualGuidance();setMessage(activePolicy()==='V4'?'V4 Target-relative 控制已选择；Target 与 Current Observation 独立。':activePolicy()==='V3'?'V3 历史实验控制已选择。':'已恢复 V2 当前默认控制。');});

closedLoopReset.addEventListener('click', () => {
  if(activePolicy()==='V4'){v4VoiceSessionRevision+=1;v4VoiceEngine.invalidate('LIVE_DISARMED');latestVoiceTelemetry=v4VoiceEngine.snapshot();v4Controller.reset();v4Projector.reset();v4VisualProjector.reset();v4XCalibration.reset();v4Trace.clear();latestV4Observation=null;latestV4Snapshot=null;latestV4Calibration=null;v4CalibrationCompletionReported=false;v4SingleGateResult=null;v4ArmRequested=false;latestV4SubjectBox=null;v4ControllerLatencies=[];renderClosedLoop();renderVisualGuidance();setMessage('V4 试验已重置；请选择 Target 后开始引导。');return;}
...
closedLoopArm.addEventListener('click', () => {
  if(activePolicy()==='V4'){const calibration=v4CalibrationLabel(),now=performance.now();if(calibration){v4Controller.reset();v4ArmRequested=false;v4Trace.clear();v4CalibrationCompletionReported=false;v4SingleGateResult=null;latestV4Calibration=v4XCalibration.start(calibration,latestV4Observation?.timestamp_ms??now);if(latestV4Observation){latestV4Snapshot=v4Controller.update(latestV4Observation,v4DirectionContext());latestV4Calibration=v4XCalibration.update(latestV4Observation);v4Trace.append(latestV4Snapshot,latestV4Calibration,latestVoiceTelemetry);}renderClosedLoop();renderVisualGuidance();setMessage(`X 标定已开始：只需让头部和双肩清晰可见并保持不动；界面明确提示“基线已取得”后，再向你自己的${calibration==='LEFT'?'左边':'右边'}移动一小步并停稳。`);return;}v4VoiceSessionRevision+=1;v4VoiceEngine.invalidate('STALE_SESSION');v4VoiceEngine.prepareFromUserGesture();latestVoiceTelemetry=v4VoiceEngine.snapshot();v4Controller.arm(now);v4ArmRequested=true;v4Trace.clear();v4XCalibration.reset();latestV4Calibration=null;v4CalibrationCompletionReported=false;v4SingleGateResult=null;if(latestV4Observation){latestV4Snapshot=v4Controller.update(latestV4Observation,v4DirectionContext());v4ArmRequested=latestV4Snapshot.armed;const presentation=deriveV4Presentation(latestV4Snapshot);latestVoiceTelemetry=v4VoiceEngine.update(presentation,{now,session_revision:v4VoiceSessionRevision,armed:latestV4Snapshot.armed,target_id:latestV4Snapshot.target.id,current_framing_ready:latestV4Snapshot.current_framing_ready,trial_success_latched:latestV4Snapshot.trial_success_latched});v4Trace.append(latestV4Snapshot,null,latestVoiceTelemetry);}else latestV4Snapshot=null;renderClosedLoop();renderVisualGuidance();setMessage(v4SingleXDeviceGate()?'05F 单次 X 复验已开始：Scale 保持目标范围，按唯一一条左右指令移动并停稳；Episode 评估后会自动解除控制。':'V4 引导已开始：一次只响应一条小步指令；无响应不会重复播报。');return;}
...
closedLoopTrace.addEventListener('click', () => {
  ...
  const context={scenario_label:scenario,generated_at_iso:new Date().toISOString(),subject_action_label:v4CalibrationLabel(),subject_x_calibration:latestV4Calibration,single_x_gate_result:v4SingleGateResult,shooting_relation:activePolicy()==='V4'?'SUBJECT_FACING_CAMERA':null,runtime_telemetry:runtimeTelemetry,session:{user_agent:navigator.userAgent,viewport_width:window.innerWidth,viewport_height:window.innerHeight,orientation:screen.orientation?.type??(window.innerHeight>=window.innerWidth?'portrait':'landscape'),camera_facing:activeFacingMode==='user'?'FRONT':activeFacingMode==='environment'?'REAR':'UNKNOWN',preview_mirror_state:activeFacingMode==='user'?'MIRRORED':activeFacingMode==='environment'?'NON_MIRRORED':'UNKNOWN',target_id:activePolicy()==='V4'?currentV4Target.id:currentTarget.id,theme_id:guidanceTheme.value,vision_target_hz:runtimeTelemetry.vision_target_hz,scheduler}};
  const policy=activePolicy();const blob = new Blob([policy==='V4'?v4Trace.json(context):policy==='V3'?v3Trace.json(context):scalarTrace.json(context)], { type: 'application/json' }); const url = URL.createObjectURL(blob);
  const link = document.createElement('a'); link.href = url; link.download = policy==='V4'?`live-p2-v4-${scenario.toLowerCase()}-${Date.now()}.json`:policy==='V3'?`live-p2-v3-${scenario.toLowerCase()}-${Date.now()}.json`:`live-p2-scale-${scenario.toLowerCase()}-${Date.now()}.json`; link.click();
  window.setTimeout(() => URL.revokeObjectURL(url), 0);
});

window.addEventListener('resize', renderCapabilities);
screen.orientation?.addEventListener('change', renderCapabilities);
document.addEventListener('visibilitychange',()=>{if(document.hidden){v4VoiceSessionRevision+=1;v4VoiceEngine.invalidate('STALE_SESSION');latestVoiceTelemetry=v4VoiceEngine.snapshot();}});
window.addEventListener('beforeunload', () => {
  stopCamera({ preserveMessage: true });
  v4VoiceEngine.dispose();
...
const v4FramingGate=new URLSearchParams(window.location.search).get('v4FramingGate');
if(v4FramingGate==='05G'){const results=V4_FRAMING_POSITION_MATRIX.map(id=>runV4FramingPositionScenario(id).at(-1)!);const pass=results.length===15&&results.every(snapshot=>snapshot.current_framing_ready&&snapshot.constraints.x_zone_relation==='IN_X_RANGE'&&snapshot.constraints.y_zone_relation==='IN_Y_RANGE');document.documentElement.dataset.v4FramingGate=pass?'PASS':'FAIL';document.documentElement.dataset.v4FramingScenarioCount=String(results.length);document.documentElement.dataset.v4FramingProfiles=String(new Set(results.map(snapshot=>snapshot.target.framing_profile_id)).size);document.documentElement.dataset.v4PositionZones=String(new Set(results.map(snapshot=>snapshot.target.position_zone)).size);const last=results.at(-1)!;controlPolicySelect.value='V4';scaleGateScenario.value=`V4_${last.target.id}`;currentV4Target=ALL_LIVE_TARGET_FIXTURES_V04[last.target.id];v4Controller.setTarget(currentV4Target);latestV4Snapshot=last;latestV4Observation=last.observation;v4Trace.clear();results.forEach(snapshot=>v4Trace.append(snapshot));poseMessage.textContent=`V4 05G SYNTHETIC MATRIX · ${results.length} SCENARIOS · NO CAMERA / NO PROVIDER`;renderClosedLoop();renderVisualGuidance();}
const voiceGate=new URLSearchParams(window.location.search).get('voiceGate');
if(voiceGate==='05I'){const result=runVoiceBrowserSmokeV01();document.documentElement.dataset.voiceGate=result.pass?'PASS':'FAIL';document.documentElement.dataset.voiceCueSequence=result.cue_sequence.join(',');document.documentElement.dataset.voiceCancelCount=String(result.cancel_count);document.documentElement.dataset.voiceOffSuppressed=String(result.voice_off_suppressed);document.documentElement.dataset.voiceProviderCalls=String(result.provider_calls);}
const v4CropGate=new URLSearchParams(window.location.search).get('v4CropGate');
if(v4CropGate==='05C'){controlPolicySelect.value='V4';scaleGateScenario.value='V4_CENTER_UPPER_BODY';currentV4Target=LIVE_TARGET_FIXTURES_V02.CENTER_UPPER_BODY;v4Controller.setTarget(currentV4Target);v4Trace.clear();const snapshots=runV4TargetScopedCropBrowserGate();poseMessage.textContent='V4 FIVE-LAYER REPLAY · OPPO 05D SANITIZED SCALARS · NO CAMERA / NO PROVIDER';snapshots.forEach((snapshot,index)=>window.setTimeout(()=>{latestV4Snapshot=snapshot;latestV4Observation=snapshot.observation;v4Trace.append(snapshot);renderClosedLoop();renderGate1Precondition();renderVisualGuidance();if(index===snapshots.length-1){const pass=snapshot.target_gap.ready&&snapshot.stage!=='ACQUIRE_REQUIRED_BODY';document.documentElement.dataset.v4CropGate=pass?'PASS':'FAIL';document.documentElement.dataset.v4CropStage=snapshot.stage;document.documentElement.dataset.v4CropGlobalBottom=String(snapshot.observation.observed_body.global_crop.bottom);document.documentElement.dataset.v4SubjectDetected=String(snapshot.observation.subject_recognition.detected);document.documentElement.dataset.v4ObservedCoverage=snapshot.observation.observed_body.coverage;document.documentElement.dataset.v4TargetGapReady=String(snapshot.target_gap.ready);}},index*120));}
const v4XDirectionGate=new URLSearchParams(window.location.search).get('v4XDirectionGate');
if(v4XDirectionGate==='05F'){controlPolicySelect.value='V4';scaleGateScenario.value='V4_CENTER_UPPER_BODY';currentV4Target=LIVE_TARGET_FIXTURES_V02.CENTER_UPPER_BODY;v4Controller.setTarget(currentV4Target);v4Trace.clear();const snapshots=runV4XDirectionBrowserGate();poseMessage.textContent='V4 X-DIRECTION REPLAY · OPPO 05E SANITIZED SCALARS · NO CAMERA / NO PROVIDER';snapshots.forEach((snapshot,index)=>window.setTimeout(()=>{latestV4Snapshot=snapshot;latestV4Observation=snapshot.observation;v4Trace.append(snapshot);renderClosedLoop();renderVisualGuidance();if(index===snapshots.length-1){const presentation=deriveV4Presentation(snapshot),decision=snapshot.direction_decision,pass=snapshot.action==='MOVE_RIGHT_SMALL'&&decision?.desired_sensor_delta_sign==='NEGATIVE'&&decision.physical_action==='SUBJECT_RIGHT'&&decision.display_axis_sign===1&&/自己的右侧/.test(presentation.primary_copy_zh);document.documentElement.dataset.v4XDirectionGate=pass?'PASS':'FAIL';document.documentElement.dataset.v4DesiredSensorSign=decision?.desired_sensor_delta_sign??'UNKNOWN';document.documentElement.dataset.v4PhysicalAction=decision?.physical_action??'UNSUPPORTED';document.documentElement.dataset.v4DisplayAxisSign=String(decision?.display_axis_sign??0);document.documentElement.dataset.v4PrimaryCopy=presentation.primary_copy_zh;}},index*120));}
const canonicalGuidanceGate=urlParameters.get('canonicalGuidanceGate');
if(canonicalGuidanceGate==='06'){const result=runCanonicalGuidanceBrowserMatrixV01();document.documentElement.dataset.canonicalGuidanceGate=result.pass?'PASS':'FAIL';document.documentElement.dataset.canonicalGuidanceCases=String(result.cases.length);document.documentElement.dataset.canonicalGuidanceFailed=result.cases.filter(item=>!item.pass).map(item=>item.id).join(',')||'NONE';document.documentElement.dataset.candidate='CANONICAL_TARGET_06';controlPolicySelect.value='V4';poseMessage.textContent=`CANONICAL TARGET 06 · BROWSER MATRIX ${result.pass?'PASS':'FAIL'} · ${result.cases.length} CASES · NO CAMERA / NO PROVIDER`;renderClosedLoop();renderVisualGuidance();}
