...
const perceptionRuntime = new PerceptionRuntime({
  ...
  onState: (state, rawMeasurement) => {
    ...
    if(activePolicy()==='V4'){
      const decisionAt=performance.now();latestV4Observation=v4Projector.project(state,Math.max(0,decisionAt-state.timestamp_ms));const started=performance.now();latestV4Snapshot=v4Controller.update(latestV4Observation,v4DirectionContext());v4ControllerLatencies.push(performance.now()-started);if(v4ControllerLatencies.length>240)v4ControllerLatencies.shift();v4Trace.append(latestV4Snapshot);if(v4SingleXDeviceGate()&&latestV4Snapshot.last_episode?.stage==='ALIGN_PRIMARY_ANCHOR'&&latestV4Snapshot.last_episode.state==='EVALUATED'){v4Controller.reset();setMessage(`05F 单次 X Episode 已结束：${latestV4Snapshot.last_episode.outcome}。控制已自动解除，请直接下载标量 Trace。`);}latestV4SubjectBox=v4VisualProjector.projectSubjectOnly(state,decisionAt);latestClosedLoop=null;latestVisualGuidance=null;
...
function renderV4Control():void{const snapshot=latestV4Snapshot;const observation=latestV4Observation??snapshot?.observation??null;const presentation=deriveV4Presentation(snapshot),calibration=v4CalibrationLabel(),calibrationCopy=calibration?`标定记录模式：先静止 2 秒，再向你自己的${calibration==='LEFT'?'左':'右'}侧移动一步并停稳，随后下载标量 Trace。`:null;v3DebugPanel.hidden=false;v2DebugGrid.hidden=true;v3Fields.policy.textContent=`CONTROL POLICY · V4 · ${snapshot?.target.id??currentV4Target.id}`;v3Fields.stage.textContent=calibration?'X_CALIBRATION':snapshot?.stage??'ACQUIRE_SUBJECT';v3Fields.framing.textContent=snapshot?.constraints.scale_relation??'UNKNOWN';v3Fields.x.textContent=snapshot?.constraints.x_relation??'UNKNOWN';v3Fields.measurement.textContent=observation?`RECOGNIZED ${observation.subject_recognition.detected?'YES':'NO'} · OBSERVED ${observation.observed_body.coverage} · GAP ${snapshot?.target_gap.ready?'READY':'OPEN'}`:'RECOGNIZED NO · OBSERVED UNKNOWN · GAP OPEN';v3Fields.stable.textContent=observation?.stable?'YES':'NO';v3Fields.action.textContent=calibration?'NONE · CALIBRATION ONLY':snapshot?.active_episode?.action??snapshot?.action??'—';v3Fields.outcome.textContent=snapshot?.last_episode?.outcome??(snapshot?.active_episode?.no_response_recorded?'NO_RESPONSE':'—');v3Fields.latency.textContent=`${percentile(v4ControllerLatencies,.5).toFixed(3)} / ${percentile(v4ControllerLatencies,.95).toFixed(3)} ms`;closedLoopFields.runtimeState.textContent=`STATE · ${calibration?'X_CALIBRATION':snapshot?.stage??'IDLE'}`;closedLoopFields.ready.textContent=`READY · ${String(snapshot?.ready??false).toUpperCase()}`;closedLoopFields.instruction.textContent=calibrationCopy??presentation.primary_copy_zh;guidanceOverlayState.textContent=calibration?'V4 · X CALIBRATION':`V4 · ${presentation.state}`;guidanceOverlayText.textContent=calibrationCopy??presentation.primary_copy_zh;closedLoopReset.textContent='重置 V4 试验';closedLoopArm.textContent=calibration?'开始 X 标定记录':v4SingleXDeviceGate()?'ARM 05F 单次 X 复验':'ARM V4 新试验';}
...
closedLoopArm.addEventListener('click', () => {
  if(activePolicy()==='V4'){const calibration=v4CalibrationLabel();if(calibration){v4Controller.reset();v4Trace.clear();latestV4Snapshot=null;renderClosedLoop();renderVisualGuidance();setMessage(`X 标定记录已开始：先静止 2 秒，再明确向你自己的${calibration==='LEFT'?'左':'右'}侧移动一步并停稳，然后下载标量 Trace。不会发出 Target 控制指令。`);return;}v4Controller.arm(performance.now());v4Trace.clear();latestV4Snapshot=null;renderClosedLoop();renderVisualGuidance();setMessage(v4SingleXDeviceGate()?'05F 单次 X 复验已 ARM：Scale 保持目标范围，按唯一一条左右指令移动并停稳；Episode 评估后会自动解除控制。':'V4 试验已 ARM：一次只响应一条小步指令；无响应只提醒，不会被判成结果。');return;}
