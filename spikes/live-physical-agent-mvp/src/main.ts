...
function renderV4Visual():void{const snapshot=latestV4Snapshot,presentation=deriveV4Presentation(snapshot),calibration=v4CalibrationLabel();visualOverlay.dataset.status=presentation.state==='READY'?'READY':presentation.action?'CORRECTING':presentation.state==='ACQUIRING'?'LOST':'TRACKING';visualGrid.classList.toggle('is-visible',guidanceGrid.checked);stopVisual.classList.remove('is-visible');const showSubjectBox=Boolean(snapshot?.observation.subject_recognition.lock_state==='LOCKED');applyProjectedBox(subjectBox,showSubjectBox?latestV4SubjectBox:null);const target=snapshot?.target??currentV4Target,targetY=target.target_anchor_y??.5,targetHeight=Math.min(.9,Math.max(.14,target.target_scale)),targetWidth=Math.min(.82,Math.max(.16,targetHeight*.62)),targetVisual:NormalizedBox={left:target.target_anchor_x-targetWidth/2,top:targetY-targetHeight/2,width:targetWidth,height:targetHeight,center_x:target.target_anchor_x,center_y:targetY},zoneHeight=Math.max(.06,(target.tolerance_y??.07)*2),zoneWidth=Math.max(.06,target.tolerance_x*2),zoneVisual:NormalizedBox={left:target.target_anchor_x-zoneWidth/2,top:targetY-zoneHeight/2,width:zoneWidth,height:zoneHeight,center_x:target.target_anchor_x,center_y:targetY};applyProjectedBox(targetBox,targetVisual);applyProjectedBox(acceptableZone,zoneVisual);debugPoseBox.classList.remove('is-visible');semanticAnchor.classList.remove('is-visible');semanticDebugCard.hidden=true;subjectLockLabel.textContent=showSubjectBox?`人物已锁定 · ${snapshot?.observation.observed_body.observed_extent??'UNKNOWN'}`:'人物锁定中';if(calibration){readyVisual.classList.remove('is-visible');directionVisual.classList.remove('is-visible');trackingVisual.textContent=latestV4Calibration?.copy_zh??`X 标定：点击开始后，先静止，再向你自己的${calibration==='LEFT'?'左边':'右边'}移动一小步并停稳`;return;}readyVisual.classList.toggle('is-visible',presentation.state==='READY');directionVisual.classList.toggle('is-visible',Boolean(presentation.action));if(presentation.action){directionLabel.textContent=presentation.overlay_copy_zh;const horizontal=presentation.action.includes('LEFT')||presentation.action.includes('RIGHT');directionIcon.textContent=horizontal?(presentation.display_axis_sign<0?'←':'→'):presentation.action==='MOVE_CLOSER_SMALL'?'＋':'−';}trackingVisual.textContent=v4ArmRequested&&!snapshot?'V4 已 ARM，等待第一帧有效观察':presentation.overlay_copy_zh;}
...
function renderV4Control():void{
  ...
  v3DebugPanel.hidden=false;v2DebugGrid.hidden=true;
...
function renderClosedLoop(): void {
  v3DebugPanel.hidden=activePolicy()==='V2';v2DebugGrid.hidden=activePolicy()!=='V2';
