...
scaleGateScenario.addEventListener('change', () => { activeGate1PreArm = null; gate1ArmAttempted = false;if(activePolicy()==='V4'&&scaleGateScenario.value.startsWith('V4_')){currentV4Target=v4CalibrationLabel()?LIVE_TARGET_FIXTURES_V02.CENTER_UPPER_BODY:v4TargetForScenario(scaleGateScenario.value);v4Controller.setTarget(currentV4Target);v4Trace.clear();latestV4Snapshot=null;closedLoopArm.textContent=v4CalibrationLabel()?'开始 X 标定记录':'ARM V4 新试验';}renderGate1Precondition();renderClosedLoop();renderVisualGuidance(); });
