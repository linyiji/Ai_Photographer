...
export class VisualGuidanceProjector {
  ...
  update(state: StructuredPerceptionState, control: ClosedLoopSnapshot, rawMeasurement: PoseMeasurement | null = null, options: { mode?: VisualServoMode; grid?: boolean; now?: number } = {}): VisualGuidanceState {
    ...
    return {
      ...
      direction_hint: directionHint, braking, ready: control.ready, ready_source: control.ready_source,
      source_timestamp: state.timestamp_ms, projection_age: Math.max(0, now - state.timestamp_ms), grid_enabled: options.grid ?? true,
      overlay_mode: options.mode ?? 'VISUAL_PLUS_TEXT', metrics: this.metrics(),
    };
  }
