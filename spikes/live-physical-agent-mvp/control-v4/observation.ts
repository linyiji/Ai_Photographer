...
import { normalizeObservedExtentV01 } from './framing-profiles.js';
...
export class HumanObservationV02Projector {
  ...
  project(state:StructuredPerceptionState,decisionAgeMs=0):Readonly<HumanObservationV02>{
    ...
    const headPoints=(head?.members??[]).filter(member=>member.visible&&finite(member.x)&&finite(member.y));const headSize=headPoints.length>=2?Math.max(Math.max(...headPoints.map(member=>member.x as number))-Math.min(...headPoints.map(member=>member.x as number)),Math.max(...headPoints.map(member=>member.y as number))-Math.min(...headPoints.map(member=>member.y as number))):null;
    const scalar=(metric:ScaleMetricV02,value:number|null,confidence:number):Readonly<ScaleEvidenceV02>=>Object.freeze({metric,value,valid:finite(value)&&value>.015,confidence});
    const headShoulderValue=headAnchor&&shoulder?Math.abs(shoulder.y-headAnchor.y)+(shoulders?.pair_width??0)*.35:null;
    const scales={HEAD_SIZE:scalar('HEAD_SIZE',headSize,head?.confidence??0),HEAD_SHOULDER_SCALE:scalar('HEAD_SHOULDER_SCALE',headShoulderValue,Math.min(head?.confidence??0,shoulders?.confidence??0)),HEAD_TO_HIP:scale('HEAD_TO_HIP',hip),HEAD_TO_KNEE:scale('HEAD_TO_KNEE',knee),HEAD_TO_ANKLE:scale('HEAD_TO_ANKLE',ankle)} as const;
    const measurementCapability:Readonly<MeasurementCapabilityV01>=Object.freeze({capability_version:'MeasurementCapabilityV01',HEAD_SIZE:measurementReadinessV01(V4_MEASUREMENT_DEFINITIONS.HEAD_SIZE,landmarkBasis,scales.HEAD_SIZE.valid,scales.HEAD_SIZE.confidence,fresh),HEAD_SHOULDER_SCALE:measurementReadinessV01(V4_MEASUREMENT_DEFINITIONS.HEAD_SHOULDER_SCALE,landmarkBasis,scales.HEAD_SHOULDER_SCALE.valid,scales.HEAD_SHOULDER_SCALE.confidence,fresh),HEAD_TO_HIP:measurementReadinessV01(V4_MEASUREMENT_DEFINITIONS.HEAD_TO_HIP,landmarkBasis,scales.HEAD_TO_HIP.valid,scales.HEAD_TO_HIP.confidence,fresh),TORSO_CENTER:measurementReadinessV01(V4_MEASUREMENT_DEFINITIONS.TORSO_CENTER,landmarkBasis,Boolean(anchors.TORSO_CENTER),anchors.TORSO_CENTER?.confidence??0,fresh),HEAD_TO_KNEE:measurementReadinessV01(V4_MEASUREMENT_DEFINITIONS.HEAD_TO_KNEE,landmarkBasis,scales.HEAD_TO_KNEE.valid,scales.HEAD_TO_KNEE.confidence,fresh),HEAD_TO_ANKLE:measurementReadinessV01(V4_MEASUREMENT_DEFINITIONS.HEAD_TO_ANKLE,landmarkBasis,scales.HEAD_TO_ANKLE.valid,scales.HEAD_TO_ANKLE.confidence,fresh)});
    ...
    const observedBody=Object.freeze({observed_body_version:'ObservedBodyStateV01' as const,state_version:state.sequence,coverage,observed_extent:normalizeObservedExtentV01(coverage),regions:bodyVisibility.regions,landmark_group_types:V4_LANDMARK_GROUP_TYPES,landmark_basis:landmarkBasis,semantic_anchors:semanticAnchors,measurement_capability:measurementCapability,scale_evidence:scaleEvidence,global_crop:Object.freeze({top:Boolean(crop?.top),bottom:Boolean(crop?.bottom),left:Boolean(crop?.left),right:Boolean(crop?.right)}),fresh,stable,confidence:framing?.confidence??0,measurement_age_ms:measurementAge});
