...
import type { BodyRegionEvidenceV01, BodyRegionV01, HumanObservationV02, LandmarkBasisScalarV01, LandmarkEvidenceStatusV01, MeasurementCapabilityV01, MeasurementReadinessV01, ScaleEvidenceV02, ScaleMetricV02, SemanticAnchorNameV01, SemanticAnchorV01, SubjectLockStateV01 } from './types.js';
...
export class HumanObservationV02Projector {
  ...
  project(state:StructuredPerceptionState,decisionAgeMs=0):Readonly<HumanObservationV02>{
    ...
    const headBasis=basis(head,crop?.top?'TOP':'NONE'),shoulderBasis=basis(shoulders,crop?.left||crop?.right?'SIDE':'NONE'),hipBasis=basis(hips,crop?.bottom?'BOTTOM':'NONE'),kneeBasis=basis(knees,crop?.bottom?'BOTTOM':'NONE'),ankleBasis=basis(ankles,crop?.bottom?'BOTTOM':'NONE');
    const torsoDerived=Boolean(shoulders?.bilateral_valid&&hips?.bilateral_valid&&shoulders.pair_center&&hips.pair_center);
    const regions:Record<BodyRegionV01,Readonly<BodyRegionEvidenceV01>>={
      ...
      UPPER_TORSO:region(torsoDerived,torsoDerived,Math.min(shoulders?.confidence??0,hips?.confidence??0),crop?.bottom?'BOTTOM':'NONE'),
      HIPS:region(Boolean(hips?.valid&&!crop?.bottom),Boolean(hips?.bilateral_valid&&!crop?.bottom),hips?.confidence??0,crop?.bottom?'BOTTOM':'NONE'),
    ...
    const readiness=(valid:boolean,confidence:number,cropped=false,partial=false):MeasurementReadinessV01=>cropped?'INVALID':valid?(fresh&&confidence>=.6?'GOOD':'MARGINAL'):partial?'MARGINAL':'INVALID';const hipPartial=hipBasis.status==='UNILATERAL_PARTIAL'||hipBasis.status==='LOW_CONFIDENCE';
    const measurementCapability:Readonly<MeasurementCapabilityV01>=Object.freeze({capability_version:'MeasurementCapabilityV01',HEAD_TO_HIP:readiness(scales.HEAD_TO_HIP.valid,scales.HEAD_TO_HIP.confidence,Boolean(crop?.top||crop?.bottom),hipPartial),TORSO_CENTER:readiness(Boolean(anchors.TORSO_CENTER),anchors.TORSO_CENTER?.confidence??0,Boolean(crop?.bottom),hipPartial),HEAD_TO_KNEE:readiness(scales.HEAD_TO_KNEE.valid,scales.HEAD_TO_KNEE.confidence,Boolean(crop?.top||crop?.bottom),kneeBasis.status==='UNILATERAL_PARTIAL'||kneeBasis.status==='LOW_CONFIDENCE'),HEAD_TO_ANKLE:readiness(scales.HEAD_TO_ANKLE.valid,scales.HEAD_TO_ANKLE.confidence,Boolean(crop?.top||crop?.bottom),ankleBasis.status==='UNILATERAL_PARTIAL'||ankleBasis.status==='LOW_CONFIDENCE')});
