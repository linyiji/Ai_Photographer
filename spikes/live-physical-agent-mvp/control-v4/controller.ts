...
import { DEFAULT_V4_DIRECTION_CONTEXT, mapSubjectPhysicalDirectionV01, physicalXActionToV4Action } from './subject-physical-direction.js';
import type { ControlEpochV04, HumanObservationV02, LiveConstraintStateV01, LiveTargetV02, SubjectPhysicalDirectionDecisionV01, V4Action, V4DirectionContextV01, V4Episode, V4Metrics, V4Outcome, V4Snapshot, V4Stage } from './types.js';

export const V4_ACTION_COPY:Readonly<Record<V4Action,string>>=Object.freeze({MOVE_LEFT_SMALL:'向你自己的左侧移动一小步，然后自然停下',MOVE_RIGHT_SMALL:'向你自己的右侧移动一小步，然后自然停下',MOVE_CAMERA_LEFT_SMALL:'请将相机向拍摄者左侧移动一点，然后停稳',MOVE_CAMERA_RIGHT_SMALL:'请将相机向拍摄者右侧移动一点，然后停稳',MOVE_CLOSER_SMALL:'靠近一小步，然后自然停下',MOVE_FARTHER_SMALL:'退后一小步，然后自然停下'});
...
export class HumanTargetRelativeServoV04 {
  ...
  update(observation:HumanObservationV02,directionContext:V4DirectionContextV01=DEFAULT_V4_DIRECTION_CONTEXT):Readonly<V4Snapshot>{
    ...
    let directionDecision:Readonly<SubjectPhysicalDirectionDecisionV01>|null=null;
    if((constraints.stage==='ADJUST_SCALE'||constraints.stage==='ALIGN_PRIMARY_ANCHOR')&&observation.fresh&&observation.stable&&observation.quality==='GOOD'&&observation.timestamp_ms-this.stageSince>=this.config.stage_persistence_ms){const candidate=this.actionFor(constraints,directionContext);directionDecision=candidate.direction;if(candidate.action)return this.issue(observation,constraints,candidate.action,candidate.direction);}
    const acquisition=constraints.stage==='ALIGN_SECONDARY_CONSTRAINT'?'当前纵向约束需要相机操作者调整，本机不伪造人物移动指令':directionDecision&&!directionDecision.supported?'当前拍摄关系无法可靠映射左右方向，已暂停水平移动提示':null;
    return this.snapshot(observation,constraints,null,acquisition,false,directionDecision);
  }
  private actionFor(c:LiveConstraintStateV01,context:V4DirectionContextV01):Readonly<{action:V4Action|null;direction:Readonly<SubjectPhysicalDirectionDecisionV01>|null}>{
    if(c.stage==='ADJUST_SCALE')return Object.freeze({action:c.scale_relation==='TOO_LOW'?'MOVE_CLOSER_SMALL':c.scale_relation==='TOO_HIGH'?'MOVE_FARTHER_SMALL':null,direction:null});
    if(c.stage!=='ALIGN_PRIMARY_ANCHOR'||c.x_relation==='IN_RANGE'||c.x_relation==='UNKNOWN')return Object.freeze({action:null,direction:null});
    const desired=c.x_relation==='TOO_LOW'?'POSITIVE' as const:'NEGATIVE' as const,direction=mapSubjectPhysicalDirectionV01(desired,c.control_actor,context);
    return Object.freeze({action:physicalXActionToV4Action(direction.physical_action),direction});
  }
  private issue(o:HumanObservationV02,c:LiveConstraintStateV01,action:V4Action,directionDecision:Readonly<SubjectPhysicalDirectionDecisionV01>|null):Readonly<V4Snapshot>{const stage=c.stage as 'ADJUST_SCALE'|'ALIGN_PRIMARY_ANCHOR';const startError=(stage==='ADJUST_SCALE'?c.scale_error_normalized:c.x_error_normalized)??0;this.episodeId+=1;this.state.issued_actions+=1;const epoch:Readonly<ControlEpochV04>=Object.freeze({trial_id:this.trialId,episode_id:this.episodeId,issued_at:o.timestamp_ms,stage,action,actor:this.target.control_actor,target_snapshot:this.target,constraint_snapshot:c,observation_state_version:o.state_version,coordinate_basis:'SENSOR_NORMALIZED_NON_MIRRORED',direction_decision:directionDecision});this.episode={trial_id:this.trialId,episode_id:this.episodeId,state:'ISSUED',stage,action,issued_at:o.timestamp_ms,response_observed:false,response_observed_at:null,movement_started_at:null,settle_started_at:null,settled_at:null,evaluated_at:null,cancelled_at:null,start_error:startError,settled_error:null,outcome:null,reminder_emitted:false,no_response_recorded:false,passive_relation_change:false,control_epoch:epoch};return this.snapshot(o,c,action,null,false,directionDecision);}
  ...
  private snapshot(o:HumanObservationV02,c:LiveConstraintStateV01,action:V4Action|null,acquisition:string|null,reminder=false,directionDecision:Readonly<SubjectPhysicalDirectionDecisionV01>|null=null):Readonly<V4Snapshot>{const targetRequirement=projectTargetMeasurementRequirementV01(this.target),targetGap=evaluateTargetObservationGapV01(o.subject_recognition,o.observed_body,targetRequirement),activeDirection=this.episode?.control_epoch.direction_decision??directionDecision;return Object.freeze({timestamp_ms:o.timestamp_ms,armed:this.armed,trial_id:this.armed?this.trialId:null,stage:this.ready?'READY_LATCHED':c.stage,observation:o,target:this.target,target_requirement:targetRequirement,target_gap:targetGap,constraints:c,action,direction_decision:activeDirection,instruction_copy_zh:action?V4_ACTION_COPY[action]:null,acquisition_copy_zh:acquisition,active_episode:this.episode?Object.freeze({...this.episode}):null,last_episode:this.lastEpisode?Object.freeze({...this.lastEpisode}):null,ready:this.ready,ready_hold_elapsed_ms:this.readyEvidenceMs,reminder_due:reminder,metrics:Object.freeze({...this.state})});}
}
