...
import { CoarseFramingController, type CoarseFramingDecision } from '../semantic-framing/coarse-controller.js';
...
export function computeDelta(state: StructuredPerceptionState, target: TargetState): DeltaState {
  ...
  const framing=state.framing; const x=framing?(framing.valid_for_precision_x?framing.anchor_x:null):state.subject.center_x;const scaleCalibration=framing?precisionScaleCalibrationFor(target,framing.body_mode,framing.scale_metric_type):null; const scale=framing?(framing.valid_for_precision_scale&&scaleCalibration?framing.scale:null):state.subject.height_ratio;
  return { x: dimension(target.center_x,x,target.tolerance_x), y: dimension(target.center_y,state.subject.center_y,target.tolerance_y,target.y_exempt), scale: dimension(scaleCalibration?.target_scale_value??target.height_ratio,scale,scaleCalibration?.target_scale_tolerance??target.tolerance_height) };
}
...
export class LocalClosedLoopEngine {
  ...
  private lastUncertaintyStateVersion:number|null=null;private coarseController=new CoarseFramingController();private coarseDecision:CoarseFramingDecision|null=null;private lastCountedCoarseEpisode=0;
  ...
  armTrial(now: number): void {
    ...
    this.readySource = null; this.verification = 'NONE'; this.settled = []; this.localFailureStreak = 0; this.lastIssuedAction = null; this.sameActionRetries = 0; this.lastTerminalStateVersion = this.lastSuppressedStateVersion = this.lastUncertaintyStateVersion = null;this.coarseController.reset();this.coarseDecision=null;this.lastCountedCoarseEpisode=0;
  }
  ...
  update(state: StructuredPerceptionState, context: ControlUpdateContext = {}): ClosedLoopSnapshot {
    ...
    if(state.framing){
      this.coarseDecision=this.coarseController.update(this.target.id,state.framing,this.trialSequence,now);this.recordCoarseOutcome();this.metrics.coarse_to_precision_handoff_count=this.coarseDecision.handoff_count;
      if(this.coarseDecision.compatibility!=='COMPATIBLE'){
        this.readyStableSince=this.passiveConfirmationSince=null;const coarse=this.coarseDecision.instruction_action;
        if(this.coarseDecision.compatibility==='UNCERTAIN'){this.runtimeState='MEASUREMENT_UNCERTAIN';this.recordUncertainty(state);return this.snapshot(now,state,delta,null,instruction);}
        this.runtimeState='FRAMING_COMPATIBILITY';if(coarse){this.instructionSequence+=1;this.metrics.framing_compatibility_instruction_count+=1;instruction=this.event(now,coarse,'SCALE');}
        const action=this.coarseDecision.episode?.action==='COARSE_MOVE_CLOSER'?'MOVE_CLOSER':'MOVE_FARTHER';return this.snapshot(now,state,delta,{kind:'SCALE',score:ISSUE_WEIGHTS.SCALE,normalized_error:1,action,action_mapping:'LOCAL_LIBRARY'},instruction);
      }
      if(!this.coarseDecision.precision_ready){this.runtimeState='ANALYZING';return this.snapshot(now,state,delta,null,instruction);}
    }
