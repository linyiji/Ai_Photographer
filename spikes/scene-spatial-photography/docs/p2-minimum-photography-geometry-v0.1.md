...
P2 is an internal capability branch of the same user-operated Scene Scan. `GeometryFrameSelector` observes the existing local frame callback independently from the P0/P1 angular keyframe sampler. It retains at most 16 160px-wide RGBA frames in transient browser memory; it never uploads or persists them.

```text
Platform Capture Adapter
  ├─ P0 angular sampler → P1 frame/direction/view candidates (max 3)
  └─ GeometryFrameSelector → SceneScanGeometryInputV01 (max 16)
