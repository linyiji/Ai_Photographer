...
export const completionToleranceDeg = (mode: SweepMode): number => ({ QUICK_SWEEP: 0.5, WIDE_SWEEP: 6, FULL_SWEEP: 8 })[mode];
export class SceneSweepRuntime {
  ...
  observeOrientation(sample: OrientationSample): boolean {
    ...
    if (this.coverage.snapshot().span_deg >= configForMode(this.session.mode).target_deg - completionToleranceDeg(this.session.mode)) this.finish(sample.timestamp_ms);
    return true;
  }
