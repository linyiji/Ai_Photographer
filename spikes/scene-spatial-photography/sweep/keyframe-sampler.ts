...
export class KeyframeSampler {
  ...
  private busy = false;
  private blurRejectionStreak = 0;
  constructor(readonly angularStepDeg = 12, readonly maxKeyframes = 12, readonly blurFallbackAfter = 12) {}
  ...
  evaluate(frame: FrameMetrics): SceneSweepKeyframe | null {
    ...
    try {
      ...
      let fallbackImperfect = false;
      if (!quality.accepted) {
        this.rejections[quality.reason!]++;
        this.blurRejectionStreak = quality.reason === 'BLUR' ? this.blurRejectionStreak + 1 : 0;
        if (quality.reason !== 'BLUR' || this.blurRejectionStreak < this.blurFallbackAfter) return null;
        fallbackImperfect = true;
        this.blurRejectionStreak = 0;
      } else this.blurRejectionStreak = 0;
      ...
      const keyframe: SceneSweepKeyframe = { ...frame, keyframe_id: `kf-${String(this.keyframes.length + 1).padStart(3, '0')}`, sequence: this.keyframes.length + 1, quality_status: 'ACCEPTED', selection_reason: fallbackImperfect ? 'FALLBACK_IMPERFECT' : nearest ? 'ANGULAR_NOVELTY' : 'INITIAL' };
      this.keyframes.push(keyframe); return keyframe;
    } finally { this.busy = false; }
  }
  reset(): void { this.keyframes.length = 0; for (const reason of Object.keys(this.rejections) as RejectionReason[]) this.rejections[reason] = 0; this.busy = false; this.blurRejectionStreak = 0; }
}
