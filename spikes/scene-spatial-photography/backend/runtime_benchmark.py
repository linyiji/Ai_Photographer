...
def main() -> None:
    ...
    for run in range(args.runs):
        ...
        request = {"scan_id": f"benchmark-{run}", "frame_set_hash": hashlib.sha256(b"".join(frames)).hexdigest(), "geometry_version": SOLVER_VERSION, "platform": "fixture", "camera_model_evidence": {"status": "KNOWN", "focal_source": "CONTROLLED", "principal_point_assumption": "IMAGE_CENTER", "distortion_assumption": "NONE", "platform_device_profile": "GENERATED_BENCHMARK", "confidence": 1.0}, "client_precheck": {"status": "POSSIBLE", "authority": "ROUTING_HINT_ONLY"}, "selected_geometry_frames": [{"frame_id": f"benchmark-{run}-{index}", "timestamp_ms": index * 250, "relative_yaw_deg": index * 3, "orientation_source": "CONTROLLED_FIXTURE", "width": 640, "height": 480, "quality": 1.0} for index in range(4)]}
