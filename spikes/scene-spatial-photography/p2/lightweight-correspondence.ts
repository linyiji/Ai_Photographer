...
const grayscale = (frame: PixelFrame): { data: Uint8Array; width: number; height: number } => {
  const scale = Math.min(1, 160 / frame.width); const width = Math.max(1, Math.round(frame.width * scale)); const height = Math.max(1, Math.round(frame.height * scale)); const gray = new Uint8Array(width * height);
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) { const sourceX = Math.min(frame.width - 1, Math.floor(x / scale)); const sourceY = Math.min(frame.height - 1, Math.floor(y / scale)); const p = (sourceY * frame.width + sourceX) * 4; gray[y * width + x] = Math.round(frame.data[p]! * .299 + frame.data[p + 1]! * .587 + frame.data[p + 2]! * .114); }
...
export const analyzeLightweightCorrespondence = (input: SceneScanGeometryInputV01): CorrespondenceDiagnostics => {
  const started = performance.now(); let detected = 0; let tracked = 0; let inliers = 0; let workingDiagonal = 0; const displacements: number[] = []; const residuals: number[] = []; const pairIndices = lightweightPairEndIndices(input.frames.length);
  for (const end of pairIndices) { const a = input.frames[end - 1]!.pixels; const b = input.frames[end]!.pixels; if (a.width !== b.width || a.height !== b.height) continue; const first = grayscale(a); const second = grayscale(b); if (first.width !== second.width || first.height !== second.height) continue; workingDiagonal = Math.hypot(first.width, first.height); const points = detectCorners(first.data, first.width, first.height); const tracks = track(first.data, second.data, first.width, first.height, points); detected += points.length; tracked += tracks.length; const globalDx = median(tracks.map(value => value.dx)); const globalDy = median(tracks.map(value => value.dy)); for (const value of tracks) { const displacement = Math.hypot(value.dx, value.dy); const residual = Math.hypot(value.dx - globalDx, value.dy - globalDy); displacements.push(displacement); residuals.push(residual); if (residual <= 2.5) inliers++; } }
  const failure = detected < 8 ? 'INSUFFICIENT_FEATURES' : tracked < 8 ? 'INSUFFICIENT_TRACKS' : null;
  return { engine: 'LIGHTWEIGHT_BLOCK_FLOW', detected_feature_count: detected, tracked_feature_count: tracked, match_retention: detected ? tracked / detected : 0, inlier_ratio: tracked ? inliers / tracked : 0, median_displacement_px: median(displacements), median_parallax_px: median(residuals), p75_parallax_px: percentile75(residuals), latency_ms: Math.round((performance.now() - started) * 10) / 10, pair_count: pairIndices.length, failure_reason: failure, working_image_diagonal_px: workingDiagonal };
};
