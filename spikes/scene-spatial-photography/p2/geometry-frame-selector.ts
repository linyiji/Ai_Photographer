...
export class GeometryFrameSelector {
  ...
  observe(metrics: FrameMetrics, pixels: PixelFrame, orientationSource: GeometryFrameObservation['orientation_source']): boolean {
    ...
    try {
      ...
      if (last && metrics.timestamp_ms - last.timestamp_ms < this.config.min_interval_ms) return false;
      if (last && Math.abs(metrics.yaw_deg - last.yaw_deg) > this.config.max_yaw_step_deg) return false;
      ...
      this.frames.push({ frame_id: `geo-${this.frames.length + 1}`, sequence: this.frames.length, timestamp_ms: metrics.timestamp_ms, relative_yaw_deg: metrics.yaw_deg, width: metrics.width, height: metrics.height, blur_score: metrics.blur_score, exposure_mean: metrics.exposure_mean, technical_quality: technicalQuality, motion_diagnostic: fingerprintDistance(last?.fingerprint, metrics.fingerprint), parallax_diagnostic: 'PENDING_POST_SCAN', orientation_source: orientationSource, pixels: copy });
